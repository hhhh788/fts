...
def run_thermal_simulation(
    pillar_temperatures: Iterable[float],
    grid_size: int = 27,
    pillar_side: int = 3,
    subgrid_side: int = 9,
    tolerance: float = 0.1,
    ...
    temperatures = np.asarray(pillar_temperatures, dtype=np.float64)
    if temperatures.shape[0] != 9:
    ...
    # 根据输入的温度列表设置恒温柱
    for i, pillar_temp in enumerate(temperatures):
        row_block = (i // 3) * subgrid_side + pillar_offset
        col_block = (i % 3) * subgrid_side + pillar_offset
        row_end, col_end = row_block + pillar_side, col_block + pillar_side
        temperature_grid[row_block:row_end, col_block:col_end] = pillar_temp
        is_pillar[row_block:row_end, col_block:col_end] = True

    # 弛豫计算主循环
    for iteration in range(max_iterations):
        previous_grid = temperature_grid.copy()

        # 使用 roll 实现周期边界条件下的邻居平均值计算
        left = np.roll(previous_grid, 1, axis=1)
        right = np.roll(previous_grid, -1, axis=1)
        top = np.roll(previous_grid, 1, axis=0)
        bottom = np.roll(previous_grid, -1, axis=0)

        averaged = (left + right + top + bottom) * 0.25
        temperature_grid = np.where(is_pillar, previous_grid, averaged)
...
if __name__ == "__main__":
    ...
    with open(output_filename, "w", newline="") as csvfile:
        ...
        for i in range(num_datasets_to_generate):
            ...
            # 2. 运行模拟得到输出
            output_grid = run_thermal_simulation(
                pillar_temperatures=input_temperatures,
                tolerance=0.1,
            )

            # 3. 将27x27的输出网格展平为一维数组
            output_flat = output_grid.ravel()
